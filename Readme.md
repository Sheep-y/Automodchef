...
Mod is compatible with all languages, but efficiency and power reports are made in English.
...
* Improved packaging machines.
* Option to load game and bypass Roboto speech.
* Higher camera, faster speeds, expanded tooltips.
...
2. Extract with 7-zip or similiar tools.

3. Move version.dll, doorstop-config.ini, and the Mod folder into game's root folder.
...
Conversingly, a layout that can clear the level with this mod may fails to do so in vanilla.
...
the mod is not being loaded.  Check version.dll and doorstop are in place and not empty,
and the Mod folder exists with at least 5 dlls.
...
The mod contains no code or assets from the game.
I wrote every single line of the mod.

It comes bundled with Unity Doorstop, HamonyX, and Mono Cecil.
